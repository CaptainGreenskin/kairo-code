...
- **Plugin system rewritten on `kairo-plugin`**. The 240-line self-built
  ...
  fetchers (LocalPath / GitHub / Npm / GitUrl / GitSubdir). New
  `:plugin install SOURCE` syntax accepts `github:owner/repo[#ref]`,
  `npm:pkg[@ver]`, `git:URL[#ref]`, `path:./local`. The Claude-Code-compatible
  `plugin.json` schema replaces the old `plugin.yaml` format.
...
- **LSP integration** via `kairo-lsp`. `DefaultLspService` is wired with the
  ...
  a tool first calls `currentDiagnostics(path)`. New
  `:lsp [status | diagnostics FILE | shutdown]` command.
...
See [`m3-task-tool-demo.md`](./m3-task-tool-demo) for a reproducible
REPL walkthrough.
